...
export default function Graph1Page() {
  ...
  const fetchTopics = async () => {
    try {
      // Check if we should load production data for debugging
      const urlParams = new URLSearchParams(window.location.search)
      const useProdData = urlParams.get('prod') === 'true'
      
      if (useProdData) {
        console.log('🔧 Attempting to load production data...')
        try {
          const response = await fetch('/production-data.json')
          if (response.ok) {
            const prodData = await response.json()
            if (prodData.topics) {
              console.log('🔧 ✅ Loading production data for debugging:', prodData.topics.length, 'topics')
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">
                Classic D3.js force-directed graph with charge, collision, and center forces
              </p>
            </div>
        ...
        {graphData.nodes.length > 0 ? (
            ...
            <div className="flex-1 bg-white rounded-2xl shadow-xl p-6 border border-gray-200">
              <div className="mb-4 flex items-center justify-between">
                <div className="text-sm text-gray-500">
                  Force-directed layout • Drag nodes to rearrange • All {graphData.links.length} arrows should be visible
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-12 text-center border border-gray-200">
            ...
            <p className="text-gray-600 mb-6">There are no topics in your knowledge base yet.</p>
          </div>
        )}
